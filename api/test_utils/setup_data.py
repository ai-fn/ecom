...
# brands = ("Docke", "ISOBOX", "ТехноНИКОЛЬ", "Yoshi")
# ctgs = (
#     "Гибкая черепица",
...
#     for ctg_name in ctgs[start:end]:
#         ctg = Category.objects.get(name=ctg_name)
#         prods = ctg.products.filter(brand=br)[:2]
#         for prod in prods:
#             prices = Price.objects.filter(product=prod)
#             print(
#                 list(
#                     prices.values(
#                         "price", "old_price", "city_group__name", "product__title"
#                     )
#                 )
#             )
#             # chars = Characteristic.objects.filter(
#             #     characteristicvalue__product=prod
#             # ).values("id", "name", "slug", "for_filtering")
#             # for char in chars:
#             #     ch_val = CharacteristicValue.objects.filter(
#             #         product=prod, characteristic__id=char["id"]
#             #     ).values("value", "slug")
#             #     char["product"] = prod.title
#             #     char["values"] = list(ch_val)
#             #     print(char)
#     start += 2
#     end += 2
